...
def setup(client):
    ...
    async def give(ctx):
        ...
        if userData != None:
            ...
            if len(msgData) != 3:
                await ctx.reply('Invalid syntax! Please use `!give {@user} {amount}`')
            else:
                try:
                    ...
                    if getUser is None:
                        await ctx.reply('Invalid user! Please use `!give {@user} {amount}`')
                    elif giveAmount > userData["money"]:
                        await ctx.reply('You don\'t have that many coins!')
                    else:
